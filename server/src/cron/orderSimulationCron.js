...
const simulateOrderProgression = async () => {
    try {
        ...
        // Only log if there are active orders to process, or keep it silent
...
const initSimulationCron = () => {
    ...
    logger.info('Order Status Simulation Cron Started (Absolute Time Mode)');
    logger.info(`Timeline: Pack @ ${TIMELINE.TO_PACKING}m, Ship @ ${TIMELINE.TO_SHIPPING}m, Deliver @ ${TIMELINE.TO_DELIVERY}m`);
};
